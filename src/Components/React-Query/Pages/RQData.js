...
import { AddUserData, DeleteRecord, UserData } from "../API/RQService";
...
function RQData() {
  ...
  const { mutate: sendVales } = useMutation(AddUserData, {
    onSuccess: (data) => {
  ...
  //Dete the record mutation
  const { mutate: deleteUserData } = useMutation(DeleteRecord, {
    onSuccess: () => {
  ...
  if (isLoading)
    return (
      <h1 className="text-center mt-[350px] text-3xl font-bold">
        is Loading...
      </h1>
    );
  return (
